...
function formatTitle(table, styles) {
  table.setAttributes(styles.tableStyle);
  for (let i = 0; i < table.getCell(0, 0).getNumChildren(); i++) {
    table.getCell(0, 0).getChild(i).setAttributes(styles.metaTitleStyle);
  }
  for (let i = 0; i < table.getCell(0, 1).getNumChildren(); i++) {
...
function formatDate(table, styles) {
  table.setAttributes(styles.tableStyle);
...
function formatSymbols(table, styles) {
  table.setAttributes(styles.tableStyle);
...
function formatDescription(table, styles) {
  table.setAttributes(styles.tableStyle);
  table.getCell(0, 0).getChild(0).setAttributes(styles.justifyStyle);
}

function formatSignatures(table, styles) {
  table.setAttributes(styles.tableStyle);
...
function formatHeader(table, styles) {
  table.setAttributes(styles.tableStyle);
...
function formatPage(body, tables) {
...
function generateTitle(table, post, department, faculty, report) {
...
function generateDates(table, advertised, closed) {
...
function generateSymbols(table) {
...
function generateDescription(table, description) {
  table.getCell(0, 0).insertParagraph(0, description);
  table.getCell(0, 0).removeChild(table.getCell(0, 0).getChild(1));
}

function generateSignatures(table) {
...
function processDegrees(retdata) {
...
function processPublications(retdata) {
  const data = retdata;
  let publications = '';
  data.forEach((item) => {
    let publication = '';
...
function processResearch(retdata) {
...
function processAwards(retdata) {
  const rawdata = JSON.parse(JSON.parse(retdata));
  const data = [];
  rawdata.forEach((item) => {
...
function processActivity(retdata) {
  const data = retdata;
  let activity = '';
  data.forEach((item) => {
    activity += `${item}\n`;
...
function processActivities(retdata) {
  const rawdata = JSON.parse(JSON.parse(retdata));
  const data = [];
  rawdata.forEach((item) => {
...
function processPresentEmployment(retdata) {
  let data = '';
...
function processExperience(retdata) {
  const data = retdata;
  let experience = '';
  data.forEach((item) => {
    experience += `${item}\n`;
...
function processExperiences(retdata) {
  const rawdata = JSON.parse(JSON.parse(retdata));
  const data = [];
  rawdata.forEach((item) => {
...
function processApplicant(retdata) {
  const obj = retdata;
...
function generateReports(table, retdata) {
  const applicants = [];
...
function updateDocument(retdata) {
  const { data } = retdata;
